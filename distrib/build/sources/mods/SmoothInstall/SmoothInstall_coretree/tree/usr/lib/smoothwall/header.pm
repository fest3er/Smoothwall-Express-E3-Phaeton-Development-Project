...
#
# $Id: header.pm 457 2010-03-02 12:05:48Z steve@domesticsecurity.com.au $
#
...
@EXPORT       = qw();
@EXPORT_OK    = qw( $language $version $webuirevision $viewsize @menu $swroot $modroot $thisscript showhttpheaders showmenu showsection openpage closepage openbigbox closebigbox openbox closebox alertbox pageinfo readvalue writevalue writehash readhash getcgihash log pipeopen age validip validmask validipormask validipandmask validport validportrange validmac validhostname validcomment basename connectedstate %tr @_validation_items getsystemid outputfile );
%EXPORT_TAGS  = (
		standard   => [@EXPORT_OK],
		);
...
$modroot = '/var/smoothwall/mods';

if ($ENV{'SCRIPT_NAME'}) {
        $thisscript = File::Basename::basename($ENV{'SCRIPT_NAME'});
        if ( $ENV{'SCRIPT_NAME'} =~ /\/mods\/(.*)\// ) {
                $mod = ",$1";
        }
}
...
# Add Mod Translations
my @modlangs = `find /var/smoothwall/mods -regex '.*/usr/lib/smoothwall/langs/${language}.pl'`;
foreach my $file (sort @modlangs) {
        open(TR, $file) or die($!);
        while(<TR>) {
                chomp($_);
		next if /^$/;
...
my @modalerts = `find /var/smoothwall/mods -regex '.*/usr/lib/smoothwall/langs/alertboxes.${language}.pl'`;
foreach my $file (sort @modalerts) {
        open(BOX, $file) or die($!);
        while(<BOX>) {
                chomp($_);
		next if /^$/;
                my @line = split(' => ', $_);
                $abouttext{$line[0]} = $line[1];
        }
        close(BOX);
}

my @modglossary = `find /var/smoothwall/mods -regex '.*/usr/lib/smoothwall/langs/glossary.${language}.pl'`;
foreach my $file (sort @modglossary) {
...
sub showmenu
{
	...
	my $menu_html;
        ...
        my @sections = `find /var/smoothwall/mods /usr/lib/smoothwall/menu -type d -regex '.*/usr/lib/smoothwall/menu/.*'`;
        my @menus;
        my @menu;
	my $selected = "";
        ...
        foreach my $section ( @sections ) {
                chomp ($section);
                my $basename = File::Basename::basename($section);
		my ( $section_title ) = ( $basename =~/\d{4}_(.*)/ );
                push @menus, $basename if (!($seen{$section_title}++));

                opendir(DIR2, "$section/");
                my @tmppages = grep {/\.list/} readdir(DIR2);
                foreach my $page (sort @tmppages) {
			next if ${$basename}{$page}++;
                        open $detail, "<$section/$page" or next;
                        ...
                        push @{$section_title}, {'page' => $page, 'title' => $title, 'href' => $link, 'active' => $active};
			@{$section_title}  = sort ({$$a{page} cmp $$b{page}} @{$section_title});
                }
		


        }
	foreach my $section (sort @menus) {
		my ( $section_title ) = ( $section =~/\d{4}_(.*)/ );
		if (scalar @{$section_title} > 0) {
				if ( $selected eq "$section_title" ){
					@menu = @{$section_title};
					$menu_html .= "<td>$first<a class='activemenu' href='/cgi-bin/$menu[ 0 ]->{'href'}'>$section_title</a></td>";
                                        ...
                                        unless ( defined $uisettings{'MENU'} and $uisettings{'MENU'} eq "off"){
                                                ...
                                                $menu_html .= &showhovermenu( @{$section_title} );
                                                $menu_html .= qq {
                                                        </div>
                                                };
                                                ...
                                                $menu_html .= qq {
                                                        ...
                                                        <td     onMouseOver="menu_show('$section_title')"
                                                                onMouseOut="menu_clear();">
                                                                $first<a class='menu' href='/cgi-bin/${$section_title}[ 0 ]->{'href'}'>$section_title</a>
                                                        </td>
                                                };
                                        } else {
...
<tr>
        <td colspan='2' class='quicklink'>
                <!-- Quicklink Section -->
                <a href='/cgi-bin/shutdown.cgi'>$tr{'ssshutdown'}</a> | <a href="javascript:displayHelp('${thisscript}${mod}');" title="This will popup a new window with the requested help file">help</a> <img src="/ui/img/help.gif" alt="">
        </td>
</tr>
