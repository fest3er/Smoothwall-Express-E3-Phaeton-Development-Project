...
my $modinfo{'MOD_NAME'} = "smoothinfo";
$modinfo{'MOD_LONG_NAME'} = "SmoothInfo for SWE 3.0 Phaeton/Roadster";
$modinfo{'MOD_DIR'} = "/var/smoothwall/$(MOD_NAME)";
$modinfo{'MOD_VERSION'} = "2.2b";
$modinfo{'MOD_INFO'} = "Generate diagnostic reports about your SmoothWall Express 3.0";
...
# mod services
if (-e "${swroot}/proxy/advanced/settings") { &readhash("${swroot}/proxy/advanced/settings", \%advproxysettings); }
if (-e "${swroot}/filtering/settings") { &readhash("${swroot}/filtering/settings", \%filteringsettings); }

unless (-z "${swroot}/dhcp/settings-green") { &readhash("${swroot}/dhcp/settings-green", \%green_dhcpsettings); }
unless (-z "${swroot}/dhcp/settings-purple") { &readhash("${swroot}/dhcp/settings-purple", \%purple_dhcpsettings); }

my $modversion = $ModDetails{'MOD_LONG_NAME'} . " v. " . $ModDetails{'MOD_VERSION'};
my $outputfile = "$MODDIR/etc/report.txt";
...
open (IFACE, "/var/smoothwall/red/iface") or warn "Could not open /var/smoothwall/red/iface: $!";
chomp ($RED = <IFACE>);
...
# IFCONFIG
my @netconf;
my @netconf_red = &pipeopen( "/sbin/ifconfig", "$netsettings{'RED_DEV'}" ) if ($netsettings{'RED_DEV'});
foreach (@netconf_red) { $_ =~ s/(\d+)\.(\d+)\.(\d+)\.(\d+)/\[\/b]\[\/color][color=#000000][b]$1\.$2\.$3\.$4\[\/b][\/color\][color=#FF0000\][b\]/g; }
...
foreach (@netconf_green) { $_ =~ s/(\d+)\.(\d+)\.(\d+)\.(\d+)/\[\/b]\[\/color][color=#000000][b]$1\.$2\.$3\.$4\[\/b][\/color\][color=#00BF00\][b\]/g; }
...
foreach (@netconf_purple) { $_ =~ s/(\d+)\.(\d+)\.(\d+)\.(\d+)/\[\/b]\[\/color][color=#000000][b]$1\.$2\.$3\.$4\[\/b][\/color\][color=#BF00FF\][b\]/g; }
...
foreach (@netconf_orange) { $_ =~ s/(\d+)\.(\d+)\.(\d+)\.(\d+)/\[\/b]\[\/color][color=#000000][b]$1\.$2\.$3\.$4\[\/b][\/color\][color=#FFBF00\][b\]/g; }
...
open (DNS1, "</var/smoothwall/red/dns1") or warn "Could not open /var/smoothwall/red/dns1: $!";
chomp($redDNS1 = <DNS1>);
if ($netsettings{'DNS1'} ne $redDNS1) { $dns1_tag = "$string"; }
$netsettings{'DNS1'} = $redDNS1;
open (DNS2, "</var/smoothwall/red/dns2") or warn "Could not open /var/smoothwall/red/dns2: $!";
chomp($redDNS2 = <DNS2>);
if ($netsettings{'DNS2'} ne $redDNS2) { $dns2_tag = "$string"; }
$netsettings{'DNS2'} = $redDNS2;
close (DNS1); close (DNS2);
open (LOCALIP, "/var/smoothwall/red/local-ipaddress") or warn "Could not open /var/smoothwall/red/local-ipaddress: $!";
chomp($redIP = <LOCALIP>);
close (LOCALIP);
if ($netsettings{'RED_ADDRESS'} ne $redIP) { $redIP_tag = "$string"; }
$netsettings{'RED_ADDRESS'} = $redIP;
open (REMOTEIP, "/var/smoothwall/red/remote-ipaddress") or warn "Could not open /var/smoothwall/red/local-ipaddress: $!";
...
open (BASE, "</usr/lib/smoothwall/langs/base.pl") || die "Couldn't open $base: $!";
my @base = <BASE>;
close (BASE);
...
if ($smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} ne 'on' && $smoothinfosettings{'WAP5'} ne 'on' && $smoothinfosettings{'WAP6'} ne 'on' && $smoothinfosettings{'SWITCH3'} ne 'on') {
print FILE "\n              |                  $purple\n";
print FILE "             WAP\n";
print FILE "              |\n";
print FILE "            WLan";
...
if ($smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH2'} ne 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'WAP5'} ne 'on' && $smoothinfosettings{'SWITCH3'} ne 'on' && $smoothinfosettings{'WAP6'} ne 'on') {
print FILE "\n                                 $purple                 |\n";
print FILE "                                                         WAP\n";
print FILE "                                                          |\n";
print FILE "                                                        WLan";
...
if ($smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'WAP5'} ne 'on' && $smoothinfosettings{'SWITCH3'} ne 'on' && $smoothinfosettings{'WAP6'} ne 'on') {
print FILE "\n                                 $purple                 |\n";
print FILE "                                                         WAP\n";
print FILE "                                                          |\n";
print FILE "                                                        WLan";
...
if ($smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} ne 'on' && $smoothinfosettings{'WAP6'} eq 'on') {
print FILE "\n              |                  (purple)\n";
print FILE "             WAP                     |\n";
print FILE "              |                     WAP\n";
print FILE "             WLan                    |\n";
...
if ($smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'WAP6'} eq 'on') {
print FILE "\n                                 (purple)                  |\n";
print FILE "                                     |                    WAP\n";
print FILE "                                    WAP                    |\n";
print FILE "                                     |                    WLan\n";
...
if ($smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'SWITCH3'} ne 'on' && $smoothinfosettings{'WAP6'} ne 'on') {
print FILE "\n              |                  $purple                 |\n";
print FILE "             WAP                                         WAP\n";
print FILE "              |                                           |\n";
...
if ($smoothinfosettings{'WAP6'} eq 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'WAP4'} ne 'on') {
print FILE "\n                                 (purple)\n";
print FILE "                                     |\n";
print FILE "                                    WAP\n";
print FILE "                                     |\n";
...
if ($smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'WAP4'} ne 'on' && $smoothinfosettings{'WAP5'} ne 'on') {
print FILE "\n                                 (purple)\n";
print FILE "                                     |\n";
print FILE "                                  Switch\n";
print FILE "                                     |\n";
...
if ($smoothinfosettings{'WAP5'} ne 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on') {
print FILE "\n              |                  (purple)                |\n";
print FILE "             WAP                     |                   WAP\n";
print FILE "              |                   Switch                  |\n";
print FILE "             WLan                    |                   WLan\n";
...
if ($smoothinfosettings{'WAP6'} eq 'on' && $smoothinfosettings{'SWITCH3'} ne 'on' && $smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on') {
print FILE "\n              |                  (purple)                 |\n";
print FILE "             WAP                     |                   WAP\n";
print FILE "              |                     WAP                   |\n";
print FILE "             WLan                    |                   WLan\n";
...
if ($smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'WAP4'} ne 'on' && $smoothinfosettings{'WAP5'} eq 'on') {
print FILE "\n                                 (purple)\n";
print FILE "                                     |\n";
...
if ($smoothinfosettings{'WAP5'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'SWITCH1'} eq 'on') {
print FILE "\n              |                  (purple)                 |\n";
print FILE "             WAP                     |                   WAP\n";
print FILE "              |           WAP <== Switch                  |\n";
print FILE "             WLan          |         |                   WLan\n";
...
if ($smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP3'} eq 'on' && $smoothinfosettings{'WAP4'} ne 'on' && $smoothinfosettings{'WAP5'} eq 'on') {
print FILE "\n              |                  (purple)\n";
print FILE "             WAP                     |\n";
print FILE "              |           WAP <== Switch\n";
print FILE "             WLan          |         |\n";
...
if ($smoothinfosettings{'SWITCH2'} ne 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'WAP5'} ne 'on') {
print FILE "\n                                 (purple)                 |\n";
print FILE "                                     |                   WAP\n";
print FILE "                                  Switch                  |\n";
print FILE "                                     |                   WLan\n";
...
if ($smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'WAP5'} ne 'on') {
print FILE "\n                                 (purple)                 |\n";
print FILE "                                     |                   WAP\n";
print FILE "                                  Switch                  |\n";
print FILE "                                     |                   WLan\n";
...
if ($smoothinfosettings{'SWITCH2'} ne 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'WAP5'} eq 'on') {
print FILE "\n                                 (purple)                 |\n";
print FILE "                                     |                   WAP\n";
print FILE "                          WAP <== Switch                  |\n";
print FILE "                           |         |                   WLan\n";
...
if ($smoothinfosettings{'SWITCH2'} eq 'on' && $smoothinfosettings{'WAP3'} ne 'on' && $smoothinfosettings{'SWITCH1'} eq 'on' && $smoothinfosettings{'SWITCH3'} eq 'on' && $smoothinfosettings{'WAP4'} eq 'on' && $smoothinfosettings{'WAP5'} eq 'on') {
print FILE "\n                                 (purple)                 |\n";
print FILE "                                     |                   WAP\n";
print FILE "                          WAP <== Switch                  |\n";
print FILE "                           |         |                   WLan\n";
...
if ($defseclevelsettings{'OPENNESS'} eq 'halfopen') { $smoothinfosettings{'SECPOLICY'} = 'Half-open'; }
if ($defseclevelsettings{'OPENNESS'} eq 'open') { $smoothinfosettings{'SECPOLICY'} = 'Open'; }
if ($defseclevelsettings{'OPENNESS'} eq 'closed') { $smoothinfosettings{'SECPOLICY'} = 'Closed'; }
...
foreach (<OUTGOING>) {
if (grep /GREEN=REJECT/, $_) { $rule_green = "$tr{'smoothinfo-traffic-originating'} GREEN is: $tr{'smoothinfo-allowed'}"; print FILE "$rule_green\n";}
if (grep /GREEN=ACCEPT/, $_) { $rule_green = "$tr{'smoothinfo-traffic-originating'} GREEN is: $tr{'smoothinfo-blocked'}"; print FILE "$rule_green\n";}
if (grep /ORANGE=REJECT/, $_) { $rule_orange = "$tr{'smoothinfo-traffic-originating'} ORANGE is: $tr{'smoothinfo-allowed'}"; print FILE "$rule_orange\n";}
if (grep /ORANGE=ACCEPT/, $_) { $rule_orange = "$tr{'smoothinfo-traffic-originating'} ORANGE is: $tr{'smoothinfo-blocked'}"; print FILE "$rule_orange\n";}
if (grep /PURPLE=REJECT/, $_) { $rule_purple = "$tr{'smoothinfo-traffic-originating'} PURPLE is: $tr{'smoothinfo-allowed'}"; print FILE "$rule_purple\n";}
if (grep /PURPLE=ACCEPT/, $_) { $rule_purple = "$tr{'smoothinfo-traffic-originating'} PURPLE is: $tr{'smoothinfo-blocked'}"; print FILE "$rule_purple\n";}
}
print FILE "\[/code\]\[/info\]";
...
if (($netsettings{'RED_TYPE'} eq 'DHCP' || $netsettings{'RED_TYPE'} eq 'PPPOE') && (-e "${swroot}/red/active")) { print FILE "[info=\"$tr{'smoothinfo-livesettings'}\"]\[quote\]@live_settings\[/quote\]\[/info\]"; }
}
			...
			if ($pid)
				...
				if (open(PID, "/proc/$pid/status"))
					...
					while (<PID>)
						...
						close PID;
						if ($status =~ /s|sleeping|r|running/i) { $process_status = 'running'; } else { $process_status = 'stopped'; }
...
if ($smoothinfosettings{'SERVICES'} eq 'on')
{
	...
	if ($SSHsettings{'ENABLE_SSH'}) { print FILE "Remote access (SSH server): $SSHsettings{'ENABLE_SSH'}\n"; }	
	if ($green_dhcpsettings{'ENABLE'}) { print FILE "DHCP server on green: $green_dhcpsettings{'ENABLE'}\n"; }
	if ($purple_dhcpsettings{'ENABLE'}) { print FILE "DHCP server on purple: $purple_dhcpsettings{'ENABLE'}\n"; }
	if ($imsettings{'ENABLE'}) { print FILE "IM Proxy: $imsettings{'ENABLE'}\n"; }
	if ($p3scansettings{'ENABLE'}) { print FILE "Pop3 Proxy: $p3scansettings{'ENABLE'}\n"; }
	if ($sipproxysettings{'ENABLE'}) { print FILE "SIP Proxy: $sipproxysettings{'ENABLE'}\n"; }
	unless (-e "${swroot}/proxy/advanced/version") {
	if ($proxysettings{'ENABLE'}) { print FILE "Web Proxy: $proxysettings{'ENABLE'}"; if ($proxysettings{'TRANSPARENT'} eq 'on') { print FILE ", in transparent mode\n"; } else { print FILE ", not in transparent mode\n"; }}
	}
	if ($snortsettings{'ENABLE_SNORT'}) { print FILE "IDS (Snort): $snortsettings{'ENABLE_SNORT'}\n"; }
	print FILE "\[/code\]\[/info\]";
}
...
if ($smoothinfosettings{'MODSERVICES'} eq 'on')
{
	...
	if ($advproxysettings{'ENABLE'}) { push (@modservices, "Advanced Web Proxy: $advproxysettings{'ENABLE'}"); if ($advproxysettings{'TRANSPARENT'} eq 'on') { push (@modservices, ", in transparent mode\n"); } else { push (@modservices, ", not in transparent mode\n"); }}
	if ($filteringsettings{'DGAV'}) { push (@modservices, "Dansguardian Web Content Filter (DGAV): $filteringsettings{'DGAV'}\n"); }
	if ($filteringsettings{'SEMF'}) { push (@modservices,  "Smoothwall Express Mail Filter (SEMF): $filteringsettings{'SEMF'}\n"); }
	if ($snortsettings{'ENABLE_GUARD'}) { push (@modservices, "Guardian Active Response (GAR): $snortsettings{'ENABLE_GUARD'}\n"); }
...
if ($smoothinfosettings{'DHCPINFO'} eq 'on' && -e "${swroot}/dhcp/enable") {
	...
	my $leaseCount = -1;
	my $dhcptmpfile = "${swroot}/smoothinfo/tempfile";
...
foreach (@chains)
	{
	if (/All chains/) { open (FIREWALL,"-|", '/usr/sbin/iptables', '-L', '-n', '-v');  last; } else { open (FIREWALL,"-|", '/usr/sbin/iptables', '-L', $_, '-n', '-v');}
@firewall = <FIREWALL>;
push (@filtering, "\n");
...
if ($smoothinfosettings{'SCREENSHOTS'} =~ /^(http|ftp)/) { $smoothinfosettings{'SCREENSHOTS'} = "\[img\]$smoothinfosettings{'SCREENSHOTS'}\[/img\]"; }
